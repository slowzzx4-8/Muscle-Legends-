...
-- Auto Grab Sky Mode (150 studs) — ajustado para personagem crescendo
local SKY_HEIGHT = 150

local function getCharacterHalfHeight(char, root)
    local half = 3
    pcall(function()
        local size = char:GetExtentsSize()
        half = math.max(size.Y / 2, 2)
    end)
    -- fallback via Humanoid HipHeight + size from leaderstats
    pcall(function()
        local sizeVal = Player:FindFirstChild("leaderstats") and Player.leaderstats:FindFirstChild("Size")
        if sizeVal then
            local n = tonumber(sizeVal.Value) or 0
            if n > 0 then
                half = math.max(half, n * 0.5 + 2)
            end
        end
    end)
    return half
...
local function syncSkyPlatform(bedrock)
    ...
    -- Sempre mesmo tamanho do bedrock (largura/comprimento)
        ...
        if skyGrabEnabled then
            pcall(function()
                ...
                if root and hum and hum.Health > 0 and bedrock and bedrock:IsA("BasePart") then
                    ...
                    local halfH = getCharacterHalfHeight(char, root)
                    -- margem extra quando fica grande (evita atravessar a plataforma)
                    local margin = math.clamp(halfH * 0.15, 2, 25)
                    local safeY = platform.Position.Y + (platform.Size.Y / 2) + halfH + margin

                    root.Anchored = true
                    root.AssemblyLinearVelocity = Vector3.zero
                    root.AssemblyAngularVelocity = Vector3.zero

                    -- Corrige se caiu abaixo ou ficou desalinhado por crescimento
                    if root.Position.Y < safeY - 1 or root.Position.Y > safeY + halfH then
                        root.CFrame = CFrame.new(root.Position.X, safeY, root.Position.Z)
                    end

                    -- Mantém dentro da área do bedrock no XZ
                    local bedSize = bedrock.Size
                    local bedPos = bedrock.Position
                    ...
                    if #fragments > 0 then
                        ...
                        -- distância de "chegou" escala com o tamanho do player
                        local reachDist = math.clamp(halfH * 1.2, 5, 80)
                        ...
                        if currentGrabTarget then
                            ...
                            local targetPos = Vector3.new(tx, safeY, tz)

                            -- Atualiza altura do tween se o player cresceu
                            local needNewTween = (currentTweenTarget ~= currentGrabTarget)
                            if currentTween and currentTween.PlaybackState == Enum.PlaybackState.Playing then
                                local curGoalY = root.Position.Y
                                if math.abs(curGoalY - safeY) > 3 then
                                    needNewTween = true
                                end
                            end

                            if needNewTween then
                                ...
                                -- velocidade sobe um pouco com o tamanho para não ficar lento quando grande
                                local speed = math.clamp(55 + halfH * 0.8, 55, 140)
