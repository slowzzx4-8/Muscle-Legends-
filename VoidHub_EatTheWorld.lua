...
AutomationTab:Toggle({
    ...
    Desc = "Creates a black transparent platform 175 studs up.",
    Default = skyGrabEnabled,
    Callback = function(State)
        skyGrabEnabled = State
        if State then
            VexUI:Notification({Title = "Sky Grab Enabled", Desc = "Platform spawned at 175 studs up.", Icon = "cloud", Duration = 4})
        else
...
EconomyTab:Toggle({ Title = "Auto Collect Cubes", Desc = "Magnetizes map cubes (Coins).", Default = cubeCoinsEnabled, Callback = function(State) cubeCoinsEnabled = State end })
...
-- Auto Grab Sky Mode (175 studs)
local SKY_HEIGHT = 175
    ...
    while task.wait(0.05) do
        if skyGrabEnabled then
            pcall(function()
                ...
                if root and hum and hum.Health > 0 and bedrock then
                    ...
                    if not platform then
                        ...
                        platform.Size = Vector3.new(bedrock.Size.X, 2, bedrock.Size.Z)
                        platform.Position = bedrock.Position + Vector3.new(0, SKY_HEIGHT, 0)
                        ...
                        -- keep height at 175 if platform already exists from older run
                        local desired = bedrock.Position + Vector3.new(0, SKY_HEIGHT, 0)
                        if (platform.Position - desired).Magnitude > 1 then
                            platform.Position = desired
                            platform.Size = Vector3.new(bedrock.Size.X, 2, bedrock.Size.Z)
                        end
                    end

                    local dynamicHeightOffset = (char:GetExtentsSize().Y / 2) + 2
                    local safeY = platform.Position.Y + dynamicHeightOffset
                    ...
                    if #fragments > 0 then
                        ...
                        if currentGrabTarget then
                            distToTarget = Vector2.new(root.Position.X - currentGrabTarget.Position.X, root.Position.Z - currentGrabTarget.Position.Z).Magnitude
                        end

                        if not currentGrabTarget or distToTarget < 3 or (tick() - lastGrabMoveTime > 4) then
                            lastGrabMoveTime = tick()
                            currentGrabTarget = fragments[math.random(1, #fragments)]
                        end

                        if currentGrabTarget then
                            local targetPos = Vector3.new(currentGrabTarget.Position.X, safeY, currentGrabTarget.Position.Z)
