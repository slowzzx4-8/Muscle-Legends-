...
local function getFortuneSpinAmount()
    ...
    pcall(function()
        ...
        local menu = gui and gui:FindFirstChild("fortuneMenu")
        local youHave = menu and menu:FindFirstChild("youHaveLabel")
        local label = youHave and youHave:FindFirstChild("spinAmountLabel")
        if label and label:IsA("TextLabel") then
            local text = tostring(label.Text or "")
            -- aceita números e "inf" / "∞"
...
local function setAutoSpinWheel(enabled)
    ...
    spawnTask("fortuneWheel", function()
        while State.running and State.autoSpinWheel do
            ...
            if spins and spins >= 1 then
                pcall(function()
                    local remote = ReplicatedStorage:FindFirstChild("rEvents")
                    remote = remote and remote:FindFirstChild("openFortuneWheelRemote")
...
-- amarelo neon só em ícones / outline (UI preta mantida)
local NEON_YELLOW = Color3.fromRGB(255, 230, 0)
local NEON_YELLOW_SOFT = Color3.fromRGB(255, 240, 80)
pcall(function()
    Window:SetTheme("Dark")
    if VoidUI and VoidUI.Theme then
        VoidUI.Theme.IconColor = NEON_YELLOW
        VoidUI.Theme.Outline = NEON_YELLOW
        VoidUI.Theme.Placeholder = NEON_YELLOW_SOFT
    end
...
Window:EditOpenButton({
    ...
    Color = ColorSequence.new{
        ...
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 255, 60)),
...
-- borda do botão abre/fecha amarelo neon
task.defer(function()
    pcall(function()
        local sg = game:GetService("CoreGui")
        for _, g in ipairs(sg:GetDescendants()) do
            if g:IsA("UIStroke") and g.Parent and g.Parent.Name and tostring(g.Parent.Name):lower():find("open") then
                g.Color = NEON_YELLOW
                g.Thickness = 1.5
            end
        end
        -- tenta via ScreenGui do Void
        for _, gui in ipairs(sg:GetChildren()) do
            if gui:IsA("ScreenGui") then
                for _, d in ipairs(gui:GetDescendants()) do
                    if d:IsA("UIStroke") and d.Parent and d.Parent:IsA("Frame") then
                        local p = d.Parent
                        if p.Size and p.Size.Y.Offset > 30 and p.Size.Y.Offset < 50 then
                            -- possível open button
                        end
                    end
                end
            end
...
-- reaplica amarelo neon nos ícones após UI montar
task.defer(function()
    task.wait(0.3)
    pcall(function()
        if VoidUI and VoidUI.Theme then
