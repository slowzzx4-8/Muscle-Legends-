...
local function doFortuneSpin()
    ...
    pcall(function()
        local rEvents = ReplicatedStorage:FindFirstChild("rEvents")
        local Event = rEvents and rEvents:FindFirstChild("openFortuneWheelRemote")
        local chances = ReplicatedStorage:FindFirstChild("fortuneWheelChances")
        local wheel = chances and (chances:FindFirstChild("Fortune Wheel") or chances["Fortune Wheel"])
        if Event and wheel then
            if Event:IsA("RemoteFunction") then
                Event:InvokeServer("openFortuneWheel", wheel)
            else
                Event:FireServer("openFortuneWheel", wheel)
            end
            ok = true
        end
    end)
    -- fallback indexing estilo Cobalt
    if not ok then
        pcall(function()
...
local function setAutoSpinWheel(enabled)
    ...
    spawnTask("fortuneWheel", function()
        while State.running and State.autoSpinWheel do
            local spins = getFortuneSpinAmount()
            -- gira se >= 1; se label não existe (nil), tenta mesmo assim
            if spins == nil or spins >= 1 then
                doFortuneSpin()
            end
            task.wait(0.5)
        end
