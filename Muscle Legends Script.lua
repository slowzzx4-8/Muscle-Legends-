--[[
    ...
    By Slowzzx4 (Completo + Correções: Kills Globais, Auto Rock Lento, Sem Mute Punch)
]]
...
local function startPunchTasks()
    ...
    spawnTask("fastPunchRock", function()
        while punchSystemActive() do
            ...
            -- INTERVALO AUMENTADO PARA EVITAR KICK (ESTAVA 0.15 AGORA É 0.7)
            task.wait(0.7)
        end
    end)
end
            ...
            while State.running and State._brawlAutoKill do
                if isInBrawlMap(LocalPlayer) then
                    for _, targetPlayer in ipairs(Players:GetPlayers()) do
                        if targetPlayer ~= LocalPlayer and isInBrawlMap(targetPlayer) then pcall(punchPlayer, targetPlayer) end
                    end
                end
                task.wait(0.05)
            end
