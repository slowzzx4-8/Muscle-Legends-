...
Tabs.Rebirths:Toggle({
    ...
    Callback = function(v)
        ...
        spawnTask("rebirthLoop", function()
            while State.running and State.rebirth.autoTarget do
                pcall(function()
                    local currentUI = 0
                    local currentStat = 0
                    pcall(function()
                        local statLabel = PlayerGui.gameGui.statsMenu.bottomStatList.rebirthsFrame.statLabel
                        currentUI = tonumber(string.match(tostring(statLabel.Text), "%d+")) or 0
                    end)
                    pcall(function()
                        local st = getStat({ "Rebirths", "Rebirth" })
                        currentStat = tonumber(st and st.Value) or 0
                    end)
                    pcall(function()
                        currentStat = math.max(currentStat, tonumber(LocalPlayer.leaderstats.Rebirths.Value) or 0)
                    end)
                    local goal = State.rebirth.target or 0
                    if goal > 0 and (currentUI >= goal or currentStat >= goal) then
                        State.rebirth.autoTarget = false
                        return
                    end
                    requestRebirth()
                end)
                if not State.rebirth.autoTarget then break end
                task.wait(1.5)
            end
        end)
    end,
...
Tabs.Trade:Button({
    ...
    Callback = function()
        ...
        pcall(function() tradePetDropdown:SetValues(getInventoryList("petsFolder")) end)
        pcall(function() tradeAuraDropdown:SetValues(getInventoryList("powerUpsFolder")) end)
    end,
})
