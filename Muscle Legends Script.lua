...
local function stopPunchAnims()
    ...
    for _, track in ipairs(hum:GetPlayingAnimationTracks()) do
        ...
        if n:find("punch", 1, true) or n:find("attack", 1, true) then
...
local function cacheRock(rock)
    ...
    for _, x in ipairs(rock:GetChildren()) do
        if x:IsA("GuiObject") or x:IsA("SurfaceGui") or x:IsA("BillboardGui")
            or x:IsA("ParticleEmitter")
            or x.Name:lower():match("particle")
            or x.Name:lower():match("emitter") then
    ...
    if rock.Parent then
        for _, x in ipairs(rock.Parent:GetChildren()) do
            if x:IsA("GuiObject") or x:IsA("SurfaceGui") or x:IsA("BillboardGui")
                or x.Name:lower():match("gui") then
                childrenToHide[x] = x.Parent
            end
        end
    end
    ...
    for _, item in pairs(folder:GetDescendants()) do
        ...
        if rock and rock:IsA("BasePart") then
            ...
            pcall(function()
                ...
                for child, _ in pairs(rockCache[rock].children) do
                    child.Parent = nil
                end
            end)
        end
...
local function attachRockToHand()
    ...
    pcall(function()
        -- totalmente transparente, pequeno, na mao
        rock.Transparency = 1
        rock.CanCollide = false
    ...
    spawnTask("fastPunchHit", function()
        while punchSystemActive() do
            pcall(function()
                ...
                -- hide anim so no punch de rock / fast punch com toggle
                if State.hidePunchAnim and State.selectedRock then
                    stopPunchAnims()
                end
            end)
            task.wait(0.01)
    ...
    spawnTask(taskName, function()
        while State.running and State[key] do
            pcall(function()
                ...
                -- NAO zera repTime: deixa animacao normal dos exercicios
                local muscleEvent = LocalPlayer:FindFirstChild("muscleEvent")
                if muscleEvent then
...
local Tabs = {
    ...
    Teleports = Window:Tab({ Title = "TP", Icon = "map-pin", Border = true }),
...
Tabs.AutoFarm:Toggle({
    Title = "Hide Punch Anim",
    Default = GetSave("HidePunchAnim", false),
    Callback = function(v)
        State.hidePunchAnim = v == true
        SetSave("HidePunchAnim", v == true)
    end,
})
...
for _, def in ipairs({
    { "Handstands", "autoHandstands", { "Handstands", "Handstand" } },
    { "Situps", "autoSitups", { "Situps", "Situp" } },
    { "Weight", "autoWeight", { "Weight" } },
    { "Lift", "autoLift", { "Pushup", "Pushups" } },
}) do
    Tabs.AutoFarm:Toggle({
        Title = def[1],
...
local function initSavedState()
    ...
    State.hidePunchAnim = GetSave("HidePunchAnim", false) == true
