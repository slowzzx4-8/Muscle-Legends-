--[[
    ...
    By Slowzzx4 (Optimized & Reorganized Edition)
    - Permanent Portals/Rebirth block
    - Industrial Rock & Gym Added
    - Decimal Size Slider
    - All requested removals applied
]]
...
local function findRock(rockDef)
    ...
    for _, item in pairs(folder:GetDescendants()) do
        if item.Name == "neededDurability" and item:IsA("ValueBase") and tonumber(item.Value) == dur then
            ...
            if rock and rock:IsA("BasePart") then cacheRock(rock) rockCache[dur] = rock return rock end
        end
    end
    return nil
end
...
local function attachRockToHand()
    ...
    pcall(function()
        ...
        rock.Transparency = 1 -- Deixa invisível enquanto ativado
...
local Window = VoidUI:CreateWindow({
    ...
    Transparent = true, Author = "By Slowzzx4", User = { Enabled = GetSave("UserEnabled", true), Anonymous = GetSave("UserAnon", true) }, Folder = "MuscleLegendsScript",
})
...
Tabs.Misc:Toggle({ Title = "Enable User Profile", Default = GetSave("UserEnabled", true), Callback = function(v) SetSave("UserEnabled", v) Window:Notify({Title = "Notice", Content = "Takes effect on re-inject.", Duration = 3}) end })
Tabs.Misc:Toggle({ Title = "Anonymous Mode", Default = GetSave("UserAnon", true), Callback = function(v) SetSave("UserAnon", v) Window:Notify({Title = "Notice", Content = "Takes effect on re-inject.", Duration = 3}) end })
...
print("[Muscle Legends] Customized Edition Loaded")