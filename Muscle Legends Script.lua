...
local function isInBrawl()
    local ok, result = pcall(function()
        ...
        if gui then
            local brawl = gui:FindFirstChild("brawlFrame") or gui:FindFirstChild("brawl")
            if brawl and brawl.Visible then return true end
        end
        -- fallback: nome do place / folder
        if workspace:FindFirstChild("brawlArena") or workspace:FindFirstChild("Brawl") then
            return true
        end
        return false
    end)
...
local function getBrawlTargets()
    local list = {}
    local myHrp = getHRP()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character then
            ...
            if tHrp and tHum and tHum.Health > 0 then
                table.insert(list, { player = p, hrp = tHrp, hum = tHum })
            end
        end
    end
    if origin then
        table.sort(list, function(a, b)
            return (a.hrp.Position - origin).Magnitude < (b.hrp.Position - origin).Magnitude
        end)
    end
    return list
end

local function stickBehind(targetHrp)
    -- colado nas costas, olhando na mesma direcao do alvo
    return targetHrp.CFrame * CFrame.new(0, 0.35, 1.0)
end
        ...
        while State.running and State.autoWinBrawl do
            if not isInBrawl() then
                ...
                if not myHrp or not myHum or myHum.Health <= 0 then
                    ...
                    local targets = getBrawlTargets()
                    if #targets == 0 then
                        task.wait(0.15)
                    else
                        -- foca o mais proximo ate morrer, depois o proximo
                        for _, t in ipairs(targets) do
                            if not State.autoWinBrawl or not isInBrawl() then break end
                            while State.autoWinBrawl and isInBrawl() do
                                myHrp = getHRP()
                                myHum = getHumanoid()
                                if not myHrp or not myHum or myHum.Health <= 0 then break end
                                if not t.player.Character then break end
                                local tHrp = t.player.Character:FindFirstChild("HumanoidRootPart")
                                local tHum = t.player.Character:FindFirstChildWhichIsA("Humanoid")
                                if not tHrp or not tHum or tHum.Health <= 0 then break end

                                pcall(function()
                                    myHrp.CFrame = stickBehind(tHrp)
                                ...
                                -- segundo hit no mesmo frame seguinte
                                RunService.Heartbeat:Wait()
                                if t.player.Character and t.player.Character:FindFirstChild("HumanoidRootPart") then
                                    local th2 = t.player.Character.HumanoidRootPart
                                    pcall(function()
                                        myHrp.CFrame = stickBehind(th2)
                                        myHrp.AssemblyLinearVelocity = Vector3.zero
                                    end)
                                    doPunch(th2)
                                end
...
Tabs.AutoFarm:Toggle({
    Title = "Join Brawl",
    Default = GetSave("AutoJoinBrawl", false),
    Callback = function(v)
        State._autoBrawl = v
        SetSave("AutoJoinBrawl", v)
...
-- Rebirth
Tabs.Rebirths:TabSection({ Title = "Fast" })
Tabs.Rebirths:Toggle({
    Title = "Fast Rebirth",
    Default = false,
    Callback = function(v)
        if v then FastFarm:Start("rebirth") else FastFarm:Stop() end
    end,
})
Tabs.Rebirths:Toggle({
    Title = "Fast Strength",
    Default = false,
    Callback = function(v)
        if v then FastFarm:Start("strength") else FastFarm:Stop() end
    end,
})
...
-- Rewards
Tabs.Rewards:TabSection({ Title = "Wheel" })
Tabs.Rewards:Toggle({
    Title = "Spin Wheel",
...
Tabs.Rewards:TabSection({ Title = "Gifts" })
Tabs.Rewards:Toggle({
        ...
        if v then
            spawnTask("claimGifts", function()
                while State.running and State.autoClaimGifts do
                    claimAllGifts()
                    task.wait(2) -- ciclo completo + pausa
                end
            end)
        end
    end,
...
Tabs.Misc:Dropdown({
    ...
    Multi = true,
    Value = GetSave("SelectedProteinItems", nil),
    Callback = function(v)
        local map = {}
