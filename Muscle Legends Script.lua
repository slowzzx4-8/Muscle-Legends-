--[[
    ...
    By Slowzzx4 (Perfected Edition)
    - Industrial Rock Added
    - Sky Kill Removed
    - Random & Extreme Speed Kills (No Spawn Protect)
    - Save System Auto-Boot Fixed (Toggles actually start on inject)
]]
...
local Config = {
    Rocks = {
        { name = "Industrial Rock", exactName = "Industrial Rock", durability = math.huge }, -- NOVO ROCK ADICIONADO
    ...
    AutoEgg = { Names = { "ProteinEgg", "Protein Egg" } },
    ServerHop = {
...
local State = {
    ...
    hideFrames = false, hideDurability = false, mainAutoSize = false, mainAutoSpeed = false,
    mainSize = 2, mainSpeed = 125, infiniteJump = false, removePortals = false, fastSpeed = false,
    antiKnockback = false, spin = false, spy = false, spyTarget = nil,
...
-- Lógica unificada para achar qualquer Rock (Industrial ou Padrão)
local function findRock(rockDef)
    if not rockDef then return nil end
    
    -- Se for o Industrial Rock (ou outro que depende do nome exato)
    if rockDef.exactName then
        if typeof(rockCache[rockDef.name]) == "Instance" and rockCache[rockDef.name].Parent then return rockCache[rockDef.name] end
        local folder = workspace:FindFirstChild("machinesFolder")
        if folder then
            local machine = folder:FindFirstChild(rockDef.exactName)
            if machine and machine:FindFirstChild("Rock") then
                cacheRockCFrame(machine.Rock)
                rockCache[rockDef.name] = machine.Rock
                return machine.Rock
            end
        end
        return nil
    ...
    -- Se for um Rock normal que depende da Durabilidade
    local dur = tonumber(rockDef.durability)
...
local function setRockSelection(rockOrNil)
    ...
    if rockOrNil then startPunchTasks() pcall(function() findRock(rockOrNil) attachRockToHand() equipPunch() end) else if not State.fastPunch then stopPunchTasks() end end
end
...
local function punchPlayer(player)
    if not player or player == LocalPlayer or isFriend(player) then return false end -- SEM VERIFICACAO DE SPAWN PROTECT AQUI
        ...
        while State.running and (State.kill.auto or State.kill.karmaMode or State.kill.targetMode) do
            pcall(function()
                if State.kill.targetMode and State.kill.target then
                    ...
                    -- Mistura aleatória para não focar em um só
                    for i = #pls, 2, -1 do
            ...
            task.wait(0.01) -- Velocidade extrema
        end
    end)
end

-- ========== Server Hop & Death Tracker ==========
...
State.setServerHop = function(enabled)
    ...
    deathCount = 0 -- Reseta contagem ao ligar
...
Tabs.Main:Slider({ Title = "Size", Value = { Min = 1, Max = 100, Default = GetSave("MainSize", 2) }, Step = 1, Callback = function(v) State.mainSize = v SetSave("MainSize", v) if State.mainAutoSize then applySizeOrSpeed("changeSize", v) end end })
...
Tabs.AutoFarm:Dropdown({ Title = "Select Protein Items", Option = proteinNamesList, Multi = true, Value = GetSave("SelectedProteinItems", nil), Callback = function(v) local map = {} if type(v) == "table" then for _, name in ipairs(v) do map[name] = true end elseif type(v) == "string" then map[v] = true end State.selectedProteinItems = map SetSave("SelectedProteinItems", v) end })
Tabs.AutoFarm:Toggle({ Title = "Auto Eat Protein", Default = GetSave("AutoEatProtein", false), Callback = function(v) State.autoEatProtein = v SetSave("AutoEatProtein", v) cancelTask("autoEatProtein") if v then spawnTask("autoEatProtein", function() while State.running and State.autoEatProtein do pcall(function() local muscleEvent = LocalPlayer:FindFirstChild("muscleEvent") if not muscleEvent then return end local selected = State.selectedProteinItems if type(selected) ~= "table" or next(selected) == nil then return end for _, def in ipairs(PROTEIN_ITEMS) do if selected[def.tool] then local tool = equipTool({def.tool}) if tool then muscleEvent:FireServer(def.event, tool) end end end end) task.wait(0.28) end end) end end })
...
Tabs.Rebirths:Toggle({ Title = "Prevent Rebirth", Default = GetSave("PreventRebirth", false), Callback = function(v) State.preventRebirth = v SetSave("PreventRebirth", v) cancelTask("prevRebirth") if v then spawnTask("prevRebirth", function() while State.running and State.preventRebirth do pcall(function() local btn = PlayerGui:FindFirstChild("gameGui") and PlayerGui.gameGui:FindFirstChild("rebirthMenu") and PlayerGui.gameGui.rebirthMenu:FindFirstChild("confirmButton") if btn then btn.Visible = false btn.Active = false end end) task.wait(0.25) end end) end end })
...
Tabs.Kills:Toggle({ Title = "Server Hop (Auto Hop 5 Deaths)", Default = GetSave("KillHop", false), Callback = function(v) SetSave("KillHop", v) State.setServerHop(v) end })
...
-- ========== INICIALIZAÇÃO DE SALVAMENTO (FIX PARA "STUCK ACTIVE") ==========
-- Aqui garantimos que todos os toggles que você fechou como "Ligado", vão realmente LIGAR SOZINHOS quando você injetar
local function initSavedState()
    ...
    if GetSave("AntiStun", false) then State.antiKnockback = true end
    ...
    if GetSave("AutoKillBrawl", false) then State._brawlAutoKill = true spawnTask("brawlAutoKill", function() while State.running and State._brawlAutoKill do local lbl = PlayerGui:FindFirstChild("gameGui") and PlayerGui.gameGui:FindFirstChild("participationRewardLabel") if LocalPlayer:FindFirstChild("currentMap") and LocalPlayer.currentMap.Value:find("Ring") and lbl and lbl.Visible then for _, p in ipairs(Players:GetPlayers()) do if p ~= LocalPlayer and p:FindFirstChild("currentMap") and p.currentMap.Value == LocalPlayer.currentMap.Value then pcall(punchPlayer, p) end end end task.wait(0.05) end end) end
    ...
    if GetSave("HideFrames", false) then setHideFrames(true) end
    if GetSave("HideDurability", false) then setHideDurability(true) end
    if GetSave("RemoveAds", false) then setRemovePortals(true) end
end
initSavedState()
...
print("[Muscle Legends] Industrial Rock + Fixed Save Initializer Edition Loaded")