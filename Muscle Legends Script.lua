...
local Tabs = {
    ...
    Stack = Window:Tab({ Title = "Stack Manager", Icon = "layers", Border = true }),
...
-- Rock list (precisa existir antes do Stack Manager)
...
local savedSlots = GetSave("StackSlots", {})
if type(savedSlots) ~= "table" then savedSlots = {} end
for i = 1, 10 do
    local v = savedSlots[i] or savedSlots[tostring(i)] or "None"
    if not table.find(StackFunctionList, v) then v = "None" end
    State.stack.slots[i] = v
end

local function applyStackFunction(name, enable)
    if name == "None" or not name then return end
    if name == "Auto Rock" then
        State._autoRock = enable
        setRockSelection(enable and rockByName[selectedRockName] or nil)
    elseif name == "Fast Punch" then
        setFastPunch(enable)
    elseif name == "Auto Tools" then
        State.autoUseTools = enable
    ...
    elseif name == "Auto Handstands" then
        setExercise("autoHandstands", enable, { "Handstands", "Handstand" }, 0.15)
    elseif name == "Auto Situps" then
        setExercise("autoSitups", enable, { "Situps", "Situp" }, 0.15)
    elseif name == "Auto Weight" then
        setExercise("autoWeight", enable, { "Weight" }, 0.15)
    elseif name == "Auto Lift" then
        setExercise("autoLift", enable, { "Pushup", "Pushups" }, 0.15)
    elseif name == "Auto Join Brawl" then
        State.brawl.autoJoin = enable
    elseif name == "Auto Kill" then
        State.brawl.autoKill = enable
    elseif name == "Auto Reset Win" then
        State.brawl.autoReset = enable
    elseif name == "Auto Win Brawl (Legacy)" then
        State.autoWinBrawl = enable
        if enable then runBrawlWin() else cancelTask("brawlWin") end
    elseif name == "Auto Rebirth" then
        State.rebirth.autoRebirth = enable
...
local function runStackManager(enable)
    State.stack.enabled = enable == true
    SetSave("StackEnabled", State.stack.enabled)
    -- desliga tudo controlado pelo stack primeiro
    for _, name in ipairs(StackFunctionList) do
        if name ~= "None" then
            applyStackFunction(name, false)
        end
    end
    if not State.stack.enabled then return end
    -- liga na ordem 1 -> 10
    for i = 1, 10 do
        local name = State.stack.slots[i]
        if name and name ~= "None" then
            applyStackFunction(name, true)
            task.wait(0.05)
        end
    end
end

Tabs.Stack:TabSection({ Title = "Priority Order (1 = first)" })
Tabs.Stack:Input({
    Title = "Rebirth objective",
    Placeholder = "18980",
    Default = tostring(GetSave("StackRebirthObjective", "") or ""),
    Callback = function(text)
        local num = tonumber(string.match(tostring(text or ""), "%d+"))
        State.rebirth.objective = num or 0
        SetSave("StackRebirthObjective", State.rebirth.objective)
    end,
})

for i = 1, 10 do
    Tabs.Stack:Dropdown({
        Title = "Slot " .. tostring(i),
        Option = StackFunctionList,
        Value = State.stack.slots[i] or "None",
        Callback = function(v)
            State.stack.slots[i] = v
            local toSave = {}
            for j = 1, 10 do toSave[j] = State.stack.slots[j] or "None" end
            SetSave("StackSlots", toSave)
            if State.stack.enabled then
...
Tabs.Stack:Toggle({
    Title = "Enable Stack Manager",
    Default = GetSave("StackEnabled", false) == true,
    Callback = function(v)
        runStackManager(v == true)
    end,
})
if GetSave("StackEnabled", false) == true then
    task.defer(function()
        runStackManager(true)
    end)
end
