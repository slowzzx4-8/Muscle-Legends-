--[[
    ...
    By Slowzzx4 (Ultimate Aggressive Edition)
    - Sky Auto Kill (10k studs)
    - Random & Extreme Speed Kills (No Spawn Protect check)
    - Auto Hop on 5 Deaths
    - Hop 1 Minute
    - Trades Removed / Hide UI Restored
]]
...
local State = {
    ...
    kill = { auto = false, skyMode = false, karmaMode = nil, protectFriends = false, targetMode = false, target = nil, serverHop = false, friendCache = {} },
...
local function setRockSelection(rockOrNil)
    ...
    if rockOrNil then startPunchTasks() pcall(function() findRockByDurability(rockOrNil.durability) attachRockToHand() equipPunch() end) else if not State.fastPunch then stopPunchTasks() end end
end
...
local function runKillLoop()
    cancelTask("killFarm")
    if not (State.kill.auto or State.kill.karmaMode or State.kill.targetMode or State.kill.skyMode) then return end
    spawnTask("killFarm", function()
        while State.running and (State.kill.auto or State.kill.karmaMode or State.kill.targetMode or State.kill.skyMode) do
            pcall(function()
                if State.kill.targetMode and State.kill.target then
                    ...
                    -- Mistura aleatória para não focar em um só (Fisher-Yates Shuffle)
                    for i = #pls, 2, -1 do
...
-- Sky Auto Kill (Plataforma a 10.000 studs)
local skyPlatform = nil
local skyCFrame = CFrame.new(0, 10000, 0)
local originalCFrame = nil

State.setSkyKill = function(enabled)
    State.kill.skyMode = enabled
    cancelTask("skyKillLoop")
    cancelTask("skyLock")
    if enabled then
        local hrp = getHRP()
        if hrp then originalCFrame = hrp.CFrame else originalCFrame = CFrame.new(0, 10, 0) end
        
        skyPlatform = Instance.new("Part")
        skyPlatform.Size = Vector3.new(50, 2, 50)
        skyPlatform.Position = skyCFrame.Position - Vector3.new(0, 5, 0)
        skyPlatform.Anchored = true
        skyPlatform.Transparency = 0.5
        skyPlatform.Parent = workspace
        
        spawnTask("skyLock", function()
            while State.running and State.kill.skyMode do
                local h = getHRP()
                if h then h.CFrame = skyCFrame h.AssemblyLinearVelocity = Vector3.zero end
                RunService.Heartbeat:Wait()
            end
        end)
        ...
        local h = getHRP()
        if h and originalCFrame then h.CFrame = originalCFrame end
        if not (State.kill.auto or State.kill.targetMode or State.kill.karmaMode) then cancelTask("killFarm") end
    end
end
...
Tabs.Kills:Toggle({ Title = "Sky Auto Kill (10.000 studs)", Default = GetSave("SkyKill", false), Callback = function(v) SetSave("SkyKill", v) State.setSkyKill(v) end })
...
-- Inicia loops caso estejam salvos como true (Fix "Stuck Active")
...
if GetSave("SkyKill", false) then State.setSkyKill(true) end
...
print("[Muscle Legends] Extreme Kills + Sky Edition Loaded")