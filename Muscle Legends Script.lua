    ...
    spawnTask("autoEatProtein", function()
        while State.running and State.autoEatProtein do
            pcall(function()
                ...
                -- só nos itens selecionados no dropdown multi
...
local function isAdPortal(obj)
    ...
    -- nomes comuns de portal / ads no Muscle Legends
    ...
    -- Billboard / SurfaceGui de anúncio
...
local function setRemovePortals(enabled)
    ...
    -- também ReplicatedStorage clones
    pcall(function()
    ...
    spawnTask("removePortals", function()
        ...
        -- re-scan periódico (portals reaparecem)
...
-- ========== Movement: Fast Speed / F-- ========== Spin / Anti Knockback ==========
...
local function punchPlayer(player)
    ...
    if ft then
        local hand = myChar:FindFirstChild("LeftHand")
            or myChar:FindFirstChild("Left Arm")
        ...
        if hand then
            pcall(ft, hrp, hand, 0)
            pcall(ft, hrp, hand, 1)
            -- também touch no torso/upper
            local torso = char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso")
            if torso then
                pcall(ft, torso, hand, 0)
                pcall(ft, torso, hand, 1)
            end
        end
    end
...
local giftSelectedItem = cfgInput("giftItem", "Protein Egg")
if giftSelectedItem ~= "Protein Egg" and giftSelectedItem ~= "Tropical Shake" then
    giftSelectedItem = "Protein Egg"
end
local giftAmount = math.clamp(tonumber(cfgInput("giftAmount", "1")) or 1, 1, 10)
...
local giftPlayerDropdown = Tabs.Gifts:Dropdown({
    ...
    Callback = function(v)
        ...
        setCfgInput("giftPlayers", table.concat(labels, "|"))
    end,
})
...
Tabs.Gifts:Dropdown({
    ...
    Value = giftSelectedItem,
    Callback = function(v)
        giftSelectedItem = v
        setCfgInput("giftItem", tostring(v or "Protein Egg"))
    end,
})
...
Tabs.Gifts:Dropdown({
    ...
    Value = tostring(giftAmount),
    Callback = function(v)
        giftAmount = math.clamp(tonumber(v) or 1, 1, 10)
        setCfgInput("giftAmount", tostring(giftAmount))
    end,
})
...
Tabs.Misc:Toggle({
    ...
    Default = cfgToggle("removePortals", true),
    Callback = function(v)
        setRemovePortals(v)
        setCfgToggle("removePortals", v)
    end,
})
