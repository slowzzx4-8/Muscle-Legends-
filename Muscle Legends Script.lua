...
local function setAutoSpinWheel(enabled)
    ...
    spawnTask("fortuneWheel", function()
        while State.running and State.autoSpinWheel do
            ...
            task.wait(4.5) -- devagar
        end
    end)
end

-- Auto Eat Protein (TOUGH Bar / ULTRA Shake / Energy Shake / Protein Shake)
local PROTEIN_ITEMS = {
        ...
        while State.running and State.autoEatProtein do
            pcall(function()
                ...
                for _, def in ipairs(PROTEIN_ITEMS) do
                    ...
                    if tool then
                        -- equip se estiver no backpack
                        local hum = getHumanoid()
                        if hum and tool.Parent ~= getCharacter() then
