...
-- Hide Frames = statEffectsGui + frames antigos do ReplicatedStorage
...
local function setHideFrames(enabled)
    ...
    if State.hideFrames then
        ...
        -- guarda junto no framesConn via wrapper
...
-- Prevent Rebirth: esconde confirmButton do rebirthMenu
...
local function requestRebirth()
    ...
    -- tenta clicar confirm se menu aberto (quando não prevent)
...
local function getFortuneSpinAmount()
    local amount = nil
    pcall(function()
        local label = nil
        -- caminho principal
        local gui = PlayerGui:FindFirstChild("fortuneWheelMenuGui")
            or PlayerGui:FindFirstChild("FortuneWheelMenuGui")
        if gui then
            label = gui:FindFirstChild("spinAmountLabel", true)
        end
        -- busca em todo PlayerGui se não achou
        if not label then
            for _, d in ipairs(PlayerGui:GetDescendants()) do
                if d.Name == "spinAmountLabel" and (d:IsA("TextLabel") or d:IsA("TextBox") or d:IsA("TextButton")) then
                    label = d
                    break
                end
            end
        ...
        if not label then
            amount = nil -- desconhecido
            return
        end
        local text = tostring(label.Text or label.ContentText or "")
        text = text:gsub(",", ""):gsub("%s+", " ")
        if text:lower():find("inf", 1, true) or text:find("∞") or text:find("unlimited", 1, true) then
            amount = math.huge
            return
        end
        -- pega o primeiro número (ex: "You have 3 spins" ou só "3")
...
-- Auto Eat Protein (só verifica se existe; não equipa)
local PROTEIN_ITEMS = {
    ...
    spawnTask("autoEatProtein", function()
        while State.running and State.autoEatProtein do
            pcall(function()
                ...
                local hasFilter = type(selected) == "table" and next(selected) ~= nil
                for _, def in ipairs(PROTEIN_ITEMS) do
                    if hasFilter and not selected[def.tool] then
                        -- skip
                    else
...
-- Mute punchSound de outros players (mantém o seu)
...
local function muteForeignPunchSound(obj)
    ...
    if isUnderLocalCharacter(obj) then return end
    -- se estiver em outro character / workspace geral
...
local function startMuteOtherPunchSounds()
    ...
    -- monitora sons que começam a tocar
...
-- sobrescreve volume do template compartilhado para clones de outros (não remove o seu)
pcall(function()
    local shared = ReplicatedStorage:FindFirstChild("shared")
    local assets = shared and shared:FindFirstChild("assets")
    local sounds = assets and assets:FindFirstChild("sounds")
    local punchSound = sounds and sounds:FindFirstChild("punchSound")
    if punchSound and punchSound:IsA("Sound") then
        -- não zera o template global; só muta instâncias de terceiros
    end
end)
...
local function lockKillPosition()
    -- desativado: auto kill sem travar o personagem
...
track(RunService.Heartbeat:Connect(function()
    ...
    if State.antiKnockback and hrp and hum and not State.fly then
        ...
        -- limita knockback vertical e horizontal
        ...
        hrp.AssemblyAngularVelocity = Vector3.zero
        -- remove BodyVelocity/AngularVelocity externos de knockback
...
-- UI Dark original + contorno cinza com tom branco
...
Tabs.FastFarm = Tabs["Auto Rebirth"] -- fast farm modes moved here
...
Tabs.AutoFarm:TabSection({ Title = "Tools" })
Tabs.AutoFarm:Toggle({
...
-- Popup de confirmação (não depende de Window:Dialog da lib)
local function showConfirm(title, desc, onConfirm, onCancel)
    task.spawn(function()
        local ok = pcall(function()
            -- tenta API da lib se existir
            if type(Window.Dialog) == "function" then
                Window:Dialog({
                    Title = title,
                    Desc = desc,
                    Buttons = {
            ...
            error("no Window.Dialog")
        end)
        if ok then return end
        -- fallback: ScreenGui simples
        pcall(function()
            local pg = LocalPlayer:FindFirstChild("PlayerGui") or PlayerGui
            local old = pg:FindFirstChild("VoidConfirmDialog")
            if old then old:Destroy() end
            local gui = Instance.new("ScreenGui")
            gui.Name = "VoidConfirmDialog"
            ...
            box.Position = UDim2.new(0.5, -140, 0.5, -75)
            ...
            titleL.Text = tostring(title or "Confirm")
            titleL.Parent = box
            local descL = Instance.new("TextLabel")
            descL.Size = UDim2.new(1, -20, 0, 48)
            descL.Position = UDim2.new(0, 10, 0, 42)
            descL.BackgroundTransparency = 1
            descL.Font = Enum.Font.Gotham
            descL.TextSize = 13
            descL.TextColor3 = Color3.fromRGB(180, 180, 190)
            descL.TextXAlignment = Enum.TextXAlignment.Left
            descL.TextYAlignment = Enum.TextYAlignment.Top
            descL.TextWrapped = true
            descL.Text = tostring(desc or "")
            descL.Parent = box
            local function mkBtn(text, xScale, color, cb)
                local b = Instance.new("TextButton")
                b.Size = UDim2.new(0.5, -16, 0, 34)
                b.Position = UDim2.new(xScale, xScale == 0 and 10 or 6, 1, -44)
            ...
            mkBtn("Cancel", 0, Color3.fromRGB(55, 55, 60), onCancel)
            mkBtn("Confirm", 0.5, Color3.fromRGB(42, 135, 62), onConfirm)
        end)
    end)
end
...
local fastRebirthToggleRef
local fastRebirthSuppress = false
fastRebirthToggleRef = Tabs.Rebirths:Toggle({
    Title = "Fast Rebirth",
    Default = false,
    Callback = function(v)
        if fastRebirthSuppress then return end
        if not v then
            if FastFarm.mode == "rebirth" then FastFarm:Stop(true) end
            return
        end
        -- desliga visual até confirmar
        fastRebirthSuppress = true
        pcall(function() if fastRebirthToggleRef then fastRebirthToggleRef:SetValue(false) end end)
        fastRebirthSuppress = false
        showConfirm(
            "Enable Fast Rebirth?",
            "Needs 7 Or 8 Pet Packs. Continue?",
            function()
                fastRebirthSuppress = true
                pcall(function() if fastRebirthToggleRef then fastRebirthToggleRef:SetValue(true) end end)
                fastRebirthSuppress = false
...
local fastStrengthToggleRef
local fastStrengthSuppress = false
fastStrengthToggleRef = Tabs.Rebirths:Toggle({
    Title = "Fast Strength",
    Default = false,
    Callback = function(v)
        if fastStrengthSuppress then return end
        if not v then
            if FastFarm.mode == "strength" then FastFarm:Stop(true) end
            return
        end
        fastStrengthSuppress = true
        pcall(function() if fastStrengthToggleRef then fastStrengthToggleRef:SetValue(false) end end)
        fastStrengthSuppress = false
        showConfirm(
            "Enable Fast Strength?",
            "Needs 7 Or 8 Pet Packs. Continue?",
            function()
                fastStrengthSuppress = true
                pcall(function() if fastStrengthToggleRef then fastStrengthToggleRef:SetValue(true) end end)
                fastStrengthSuppress = false
...
local function setRebirthSupportTools(active)
    -- Fast Weight + Size 1 junto com Auto/Infinite Rebirth
    if active then
...
local autoRebirthToggleRef
local autoRebirthSuppress = false
autoRebirthToggleRef = Tabs.Rebirths:Toggle({
    Title = "Auto Rebirth",
    Default = false,
    Callback = function(v)
        if autoRebirthSuppress then return end
        if not v then
            State.rebirth.autoRebirth = false
            setCfgToggle("smartAutoRebirth", false)
            cancelTask("rebirthLoop")
            cancelTask("smartAutoRebirthTask")
            setRebirthSupportTools(false)
            return
        end
        autoRebirthSuppress = true
        pcall(function() if autoRebirthToggleRef then autoRebirthToggleRef:SetValue(false) end end)
        autoRebirthSuppress = false
        showConfirm(
            "Enable Auto Rebirth?",
            "Will Rebirth Until Objective. Continue?",
            function()
                State.rebirth.autoRebirth = true
                State.rebirth.autoTarget = false
                State.rebirth.infinite = false
                setCfgToggle("smartAutoRebirth", true)
                cancelTask("rebirthLoop")
                cancelTask("smartAutoRebirthTask")
                setRebirthSupportTools(true)
                autoRebirthSuppress = true
                pcall(function() if autoRebirthToggleRef then autoRebirthToggleRef:SetValue(true) end end)
                autoRebirthSuppress = false
...
local infiniteRebirthToggleRef
local infiniteRebirthSuppress = false
infiniteRebirthToggleRef = Tabs.Rebirths:Toggle({
    Title = "Infinite Rebirths",
    Default = false,
    Callback = function(v)
        if infiniteRebirthSuppress then return end
        if not v then
            State.rebirth.infinite = false
            cancelTask("rebirthLoop")
            cancelTask("smartAutoRebirthTask")
        ...
        infiniteRebirthSuppress = true
        pcall(function() if infiniteRebirthToggleRef then infiniteRebirthToggleRef:SetValue(false) end end)
        infiniteRebirthSuppress = false
        showConfirm(
            "Enable Infinite Rebirths?",
            "Will Rebirth Continuously. Continue?",
            function()
                State.rebirth.infinite = true
                State.rebirth.autoTarget = false
                State.rebirth.autoRebirth = false
                cancelTask("rebirthLoop")
                cancelTask("smartAutoRebirthTask")
                setRebirthSupportTools(true)
                infiniteRebirthSuppress = true
                pcall(function() if infiniteRebirthToggleRef then infiniteRebirthToggleRef:SetValue(true) end end)
                infiniteRebirthSuppress = false
...
-- ========== GIFTS ==========
Tabs.Gifts:TabSection({ Title = "Send Protein" })
local giftSelectedPlayers = {}
local giftSelectedItem = nil
local giftAmount = 1

local giftItemOptions = {
    "Protein Egg",
    "Tropical Shake",
    "TOUGH Bar",
    "ULTRA Shake",
...
Tabs.Gifts:Dropdown({
    Title = "Select Item",
    Option = giftItemOptions,
    Value = giftItemOptions[1],
    Callback = function(v)
        giftSelectedItem = v
    end,
})
giftSelectedItem = giftItemOptions[1]
...
Tabs.Gifts:Dropdown({
    ...
    Value = "1",
    Callback = function(v)
        giftAmount = math.clamp(tonumber(v) or 1, 1, 10)
    end,
})
...
Tabs.Gifts:Button({
    ...
    Callback = function()
        ...
        if not giftSelectedItem then
            Window:Notify({ Title = "Gifts", Content = "Select An Item", Icon = "gift", Duration = 2 })
            return
        end
        ...
        if not remote then
            Window:Notify({ Title = "Gifts", Content = "Gift Remote Not Found", Icon = "x", Duration = 2 })
            return
        end
            ...
            for _, target in ipairs(giftSelectedPlayers) do
                ...
                for i = 1, giftAmount do
                    ...
                    if not item then
                        ...
                        item = bp and bp:FindFirstChild(giftSelectedItem)
                    end
                    if not item then
                        local char = getCharacter()
                        item = char and char:FindFirstChild(giftSelectedItem)
                    end
                    -- protein eggs from Config names
                    if not item and giftSelectedItem == "Protein Egg" then
                        for _, name in ipairs(Config.AutoEgg.Names or { "Protein Egg" }) do
                            item = folder and folder:FindFirstChild(name)
                    ...
                    pcall(function()
                        if remote:IsA("RemoteFunction") then
                            remote:InvokeServer("giftRequest", target, item)
                        else
                            remote:FireServer("giftRequest", target, item)
                        end
                    end)
                    task.wait(0.16)
                end
...
Tabs.Gifts:Toggle({
    Title = "Spin Wheel",
    Default = cfgToggle("spinWheel", false),
    Callback = function(v)
        setAutoSpinWheel(v)
        setCfgToggle("spinWheel", v)
    end,
})
-- ========== TRADE ==========
...
State.tradeExt = State.tradeExt or {
    sendToPlayer = nil,
    acceptFromPlayer = nil,
    selectedPet = nil,
    selectedAura = nil,
    amount = 1,
}
...
local tradeSendDropdown = Tabs.Trade:Dropdown({
    ...
    Option = getPlayerNames(),
    Value = nil,
    Callback = function(v)
        State.tradeExt.sendToPlayer = v
    end,
})

Tabs.Trade:Toggle({
    Title = "Auto Send Request",
    Default = cfgToggle("autoSendRequest", false),
    Callback = function(v)
        State._autoSendRequest = v == true
        setCfgToggle("autoSendRequest", v)
        ...
        spawnTask("autoSendRequest", function()
            while State.running and State._autoSendRequest do
                ...
                if name then
                    ...
                    if target then
                        pcall(function()
                            ReplicatedStorage.rEvents.tradingEvent:FireServer("sendTradeRequest", target)
                        end)
                    end
                end
                task.wait(1)
...
local tradeAcceptDropdown = Tabs.Trade:Dropdown({
    Title = "Accept Invites From",
    Option = getPlayerNames(),
    Value = nil,
    Callback = function(v)
        State.tradeExt.acceptFromPlayer = v
    end,
})

Tabs.Trade:Toggle({
    Title = "Auto Accept Invite",
    Default = cfgToggle("autoAcceptInvite", false),
    Callback = function(v)
        State._autoAcceptInvite = v == true
        setCfgToggle("autoAcceptInvite", v)
...
local tradePetDropdown = Tabs.Trade:Dropdown({
    Title = "Select Pet",
    Option = getInventoryList("petsFolder"),
    Value = nil,
    Callback = function(v)
        State.tradeExt.selectedPet = v
    end,
})

local tradeAuraDropdown = Tabs.Trade:Dropdown({
    Title = "Select Aura",
    Option = getInventoryList("powerUpsFolder"),
    Value = nil,
    Callback = function(v)
        State.tradeExt.selectedAura = v
    end,
})
...
Tabs.Trade:Dropdown({
    ...
    Value = "1",
    Callback = function(v)
        State.tradeExt.amount = tonumber(v) or 1
    end,
})
...
Tabs.Trade:Toggle({
    Title = "Auto Put Pet",
    Default = cfgToggle("autoPutPet", false),
    Callback = function(v)
        State._autoPutPet = v == true
        setCfgToggle("autoPutPet", v)
            ...
            while State.running and State._autoPutPet do
                if State.tradeExt.selectedPet then
                    ...
                    for _, pet in ipairs(pets) do
                        pcall(function()
                            ReplicatedStorage.rEvents.tradingEvent:FireServer("offerItem", pet)
                        end)
                        task.wait(0.1)
                    end
                end
...
Tabs.Trade:Toggle({
    Title = "Auto Put Aura",
    Default = cfgToggle("autoPutAura", false),
    Callback = function(v)
        State._autoPutAura = v == true
        setCfgToggle("autoPutAura", v)
            ...
            while State.running and State._autoPutAura do
                if State.tradeExt.selectedAura then
                    ...
                    for _, aura in ipairs(auras) do
                        pcall(function()
                            ReplicatedStorage.rEvents.tradingEvent:FireServer("offerItem", aura)
                        end)
                        task.wait(0.1)
                    end
                end
...
Tabs.Trade:Toggle({
    ...
    Default = cfgToggle("autoConfirmFinal", false),
    Callback = function(v)
        State._autoConfirmFinal = v == true
        setCfgToggle("autoConfirmFinal", v)
        ...
        spawnTask("autoConfirmFinal", function()
            while State.running and State._autoConfirmFinal do
                pcall(function()
                    ...
                    pcall(function()
                        acceptedLabel = gameGui.tradePanel.otherOfferMenu.itemFrames.backImageFolder.acceptedCover.acceptedLabel
                    end)
                    if not acceptedLabel then
                        acceptedLabel = gameGui:FindFirstChild("acceptedLabel", true)
                    end
                    if acceptedLabel and acceptedLabel.Visible then
                        for _ = 1, 3 do
                            ReplicatedStorage.rEvents.tradingEvent:FireServer("acceptTrade")
                            task.wait(0.1)
                        end
                        while State.running and State._autoConfirmFinal and acceptedLabel.Visible do
                            task.wait(0.2)
                        end
                    end
                end)
...
-- Refresh automático de listas (players / pets / auras)
spawnTask("autoRefreshLists", function()
    while State.running do
        pcall(function()
            local players = getPlayerNames and getPlayerNames() or nil
            local playersDisp = playerList and playerList() or nil
            if tradeSendDropdown and tradeSendDropdown.SetValues and players then
                tradeSendDropdown:SetValues(players)
            end
            if tradeAcceptDropdown and tradeAcceptDropdown.SetValues and players then
                tradeAcceptDropdown:SetValues(players)
            end
            ...
            if killPlayerDropdown and killPlayerDropdown.SetValues and playersDisp then
                killPlayerDropdown:SetValues(playersDisp)
            end
            if viewPlayerDropdown and viewPlayerDropdown.SetValues and playersDisp then
                viewPlayerDropdown:SetValues(playersDisp)
            end
            if giftPlayerDropdown and giftPlayerDropdown.SetValues and playersDisp then
                giftPlayerDropdown:SetValues(playersDisp)
            end
        end)
        task.wait(5)
    end
...
Tabs.Misc:Toggle({
    Title = "Hide Frames",
    Default = cfgToggle("hideFrames", false),
    Callback = function(v)
        setHideFrames(v)
        setCfgToggle("hideFrames", v)
    end,
})
Tabs.Misc:Toggle({
