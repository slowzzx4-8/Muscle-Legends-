--[[ 
    ...
    Modified by Slowzzx4
    - Lag fixes (equip animations/emotes)
    - Footagesus / Lucide icons support
]]


if _G.EmotesGUIRunning then
    getgenv().Notify({
        Title = 'Slowzzx4 | Emote',
...
-- Footagesus Icons (lucide) — WindUI-style icon support
local IconsV2 = nil
pcall(function()
...
local function GetLucideIcon(name)
    if not IconsV2 or not name then return nil end
        ...
        for _, assetId in pairs(assetIds) do
            ...
            if success and objects then
                ...
                for _, obj in pairs(objects) do
                    ...
                    -- destroy immediately without parenting to Workspace (avoids lag spikes)
...
function SafeLoad(url, name)
    ...
    if not func then
        warn("Slowzzx4 | SafeLoad: Failed to parse " .. (name or "script") .. ": " .. tostring(err))
        return function() end
    end

    local ok, res = pcall(func)
    if not ok then
        warn("Slowzzx4 | SafeLoad: Error executing " .. (name or "script") .. ": " .. tostring(res))
        return function() end
    end
    return res
end
        ...
        if isfile(path) then
            ...
            if success and content and content ~= "" then
                ...
                if low:find("<!doctype") or low:find("<html") or low:find("<head") then
                    warn("Slowzzx4 | GetAsset: Downloaded content appears to be HTML. Link might be incorrect: " .. targetUrl)
                    return ""
                end
...
function ApplyTheme(themeData)
    ...
    if not themeData then
        warn("Slowzzx4 | ApplyTheme: themeData is nil. Falling back to Default.")
    ...
    if not ok then
        warn("Slowzzx4 | ApplyTheme error: " .. tostring(err))
    end
end
...
State.enterCustomAnimationEditor = function(category, animName)
    ...
    if State.currentCustomAnimationName == "Default" then
        getgenv().Notify({ Title = "Slowzzx4 | Error", Content = "Cannot edit Default Animation set. Create a new one!", Duration = 3 })
        return
    end
    ...
    getgenv().Notify({ Title = "Slowzzx4 | Animation Editor", Content = "Select an animation from the wheel to set for " .. animName, Duration = 5 })
end
...
SettingsLib.AddIconButton(CustomAnimMgtContainer, "78317476576895", function()
    ...
    imp.MouseButton1Click:Connect(function()
        ...
        if s and type(d) == "table" then
            ...
            if refreshCustomAnimationState then refreshCustomAnimationState(false) end
            popup:Destroy()
            getgenv().Notify({ Title = "Slowzzx4 | Animation", Content = "Imported custom animations", Duration = 3 })
        else
            getgenv().Notify({ Title = "Error", Content = "Invalid JSON", Duration = 3 })
        end
    end)
...
SettingsLib.AddIconButton(EmotePageMgtContainer, "78317476576895", function() 
    ...
    imp.MouseButton1Click:Connect(function()
        ...
        if s and type(d) == "table" and d.Type == "EmotePageSet" and type(d.Data) == "table" then
            ...
            popup:Destroy()
            getgenv().Notify({ Title = "Slowzzx4 | Page", Content = "Imported Emote page", Duration = 3 })
        else
        ...
        if s and type(d) == "table" and d.Type then
            ...
            if d.Themes and (typeStr == "All" or typeStr == "Themes") then
                ...
                if themeToApply then
                    ...
                    warn("Slowzzx4 | Missing Default theme during import fallback")
                end
            end
...
function stopEmotes()
    ...
    for _, track in ipairs(humanoid:GetPlayingAnimationTracks()) do
        -- evita parar idle/walk/run (só emotes/action)
        ...
        if prio == Enum.AnimationPriority.Action
            ...
            or track == State.currentEmoteTrack then
            track:Stop(0.05)
        end
    end
end
...
State.EmoteResolveCache = State.EmoteResolveCache or {}
...
function resolveEmoteToAnimationId(emoteId)
    ...
    if State.EmoteResolveCache[idStr] then
        return State.EmoteResolveCache[idStr]
    end
    ...
    if ok and type(objects) == "table" then
        ...
        -- NÃO parentar no Workspace (isso causa lag/hitch)
        for _, obj in ipairs(objects) do
            local found = findAnimId(obj)
            pcall(function() obj:Destroy() end)
            if found then
                State.EmoteResolveCache[idStr] = found
                return found
            end
        end
    end
    State.EmoteResolveCache[idStr] = fallbackId
    return fallbackId
end
...
function updateAnimations()
    ...
    if MarkGUIDirty then MarkGUIDirty() end
...
toggleFavorite = function(emoteId, emoteName)
    ...
    if found then
        ...
        getgenv().Notify({
            Title = 'Slowzzx4 | Favorite System',
...
applyAnimation = function(animationData)
    ...
    if not animate or not humanoid then
        getgenv().Notify({
            Title = 'Slowzzx4 | Animation Error',
    ...
    -- não para idle/walk (só action / emote atual) — reduz hitch ao equipar
    ...
    if index == 1 and randomActive then
        ...
        if State.customAnimationEditorActive then
            ...
            if State.CustomAnimations.Sets[State.currentCustomAnimationName] and cat and name then
                ...
                State.SaveCustomAnimations(State.CustomAnimations)
                getgenv().Notify({ Title = "Slowzzx4 | Saved", Content = "Saved " .. name, Duration = 3 })
                if State.RefreshCustomAnimUI then State.RefreshCustomAnimUI() end
                if refreshCustomAnimationState then refreshCustomAnimationState(true) end
                State.exitCustomAnimationEditor()
            end
    ...
    task.spawn(function()
        while true do
            ...
            if emoteData then
                ...
                getgenv().Notify({Title = 'Slowzzx4 | Emote', Content = "Emotes loaded", Duration = 3})
                return
            end
            task.wait(3)
        end
...
function onCharacterAdded(character)
    ...
    if getgenv().autoReloadEnabled and getgenv().lastPlayedAnimation then
        task.spawn(function()
            ...
            getgenv().Notify({
                Title = 'Slowzzx4 | Auto Reload Animation',
...
enterHUDEditor = function()
    ...
    table.insert(HUD.Connections, exportBtn.MouseButton1Click:Connect(function()
        ...
        setclipboard(HttpService:JSONEncode(data))
        getgenv().Notify({ Title = "Slowzzx4 | HUD Editor", Content = "HUD settings copied", Duration = 2 })
    end))
    ...
    table.insert(HUD.Connections, importBtn.MouseButton1Click:Connect(function()
        ...
        imp.MouseButton1Click:Connect(function()
            ...
            if s and type(d) == "table" then
                ...
                popup:Destroy()
                getgenv().Notify({ Title = "Slowzzx4 | HUD Editor", Content = "HUD settings imported", Duration = 2 })
            else
                getgenv().Notify({ Title = "Error", Content = "Invalid HUD JSON", Duration = 3 })
            end
        end)
    ...
    table.insert(HUD.Connections, resetBtn.MouseButton1Click:Connect(function()
        ...
        getgenv().Notify({ Title = "Slowzzx4 | HUD Editor", Content = "🔄 All designs and frames have been fully reset", Duration = 3 })
    end))
    ...
    table.insert(HUD.Connections, deleteBtn.MouseButton1Click:Connect(function()
        ...
        ApplyUIVisibility()
        pcall(function() updateGUIColors() end)
        getgenv().Notify({ Title = "Slowzzx4 | HUD Editor", Content = "Custom Frame deleted", Duration = 2 })
    end))
    ...
    table.insert(HUD.Connections, addBtn.MouseButton1Click:Connect(function()
        ...
        getgenv().Notify({ Title = "Slowzzx4 | HUD Editor", Content = "Custom Frame added!", Duration = 2 })
    end))

    getgenv().Notify({ Title = "Slowzzx4 | HUD Editor", Content = "Drag elements to reposition", Duration = 5 })
end
...
-- Throttle: updateGUIColors/enforceImages NÃO rodam todo frame (principal causa de lag)
State._lastGuiColorTick = 0
State._guiColorDirty = true
State._lastGuiRecreateTick = 0
local GUI_COLOR_INTERVAL = 0.35
local GUI_RECREATE_INTERVAL = 0.5

RunService.Heartbeat:Connect(function()
    local now = tick()
    if not State.isGUICreated then
        if now - (State._lastGuiRecreateTick or 0) >= GUI_RECREATE_INTERVAL then
            State._lastGuiRecreateTick = now
            checkAndRecreateGUI()
        end
        return
    end

    -- images: só quando targetImages mudou ou interval
    if State._guiColorDirty or (now - (State._lastGuiColorTick or 0) >= GUI_COLOR_INTERVAL) then
        State._lastGuiColorTick = now
        State._guiColorDirty = false
        pcall(updateGUIColors)
        pcall(enforceImages)
    end
end)

-- helper para marcar UI dirty (chame após mudar tema/página)
function MarkGUIDirty()
    State._guiColorDirty = true
end
